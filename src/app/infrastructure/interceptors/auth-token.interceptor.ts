...
export class AuthTokenInterceptor implements HttpInterceptor {
  ...
  intercept(
    ...
    console.log("🔄 AuthTokenInterceptor - Interceptando petición:", {
      ...
      originalHeaders: request.headers.keys().map(k => ({ [k]: request.headers.get(k) }))
    });
    ...
    if (isPlatformBrowser(this.platformId)) {
      ...
      if (token) {
        ...
        console.log("🔎 Headers enviados:", authRequest.headers.keys().map(k => ({ [k]: authRequest.headers.get(k) })));
