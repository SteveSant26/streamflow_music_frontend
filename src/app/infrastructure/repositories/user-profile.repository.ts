...
export class UserProfileRepository implements IUserProfileRepository {
  ...
  getCurrentUserProfile(): Observable<GetUserProfileDto> {
    console.log('🌐 UserProfileRepository: GET', API_CONFIG_PROFILE.profileMe.get);
    return this.apiService.get<GetUserProfileDto>(API_CONFIG_PROFILE.profileMe.get);
  }

  updateUserProfile(updateData: UpdateUserProfileDto): Observable<UserProfileResponse> {
    console.log('🌐 UserProfileRepository: POST', API_CONFIG_PROFILE.profileMe.post, updateData);
    return this.apiService.post<UserProfileResponse>(API_CONFIG_PROFILE.profileMe.post, updateData);
  }

  uploadProfilePicture(file: File): Observable<{ profile_picture: string }> {
    console.log('🌐 UserProfileRepository: POST (FormData)', API_CONFIG_PROFILE.profileMe.uploadProfilePicture);
