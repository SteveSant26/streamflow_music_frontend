...
export class AuthRepositoryImpl extends AuthRepository {
  ...
  async login(credentials: LoginCredentials): Promise<AuthResult> {
    ...
    if (error) {
      ...
      if (error.message.includes('Email not confirmed')) {
        throw new Error(
          'Tu cuenta no ha sido confirmada. Por favor, revisa tu email y confirma tu cuenta.',
        );
      }
    ...
    if (!data.user) {
      ...
      throw new Error(
        'Inicio de sesión fallido: No se recibieron datos del usuario',
      );
    }
  ...
  async register(credentials: RegisterCredentials): Promise<AuthResult> {
    ...
    try {
      ...
      console.log(
        'Login inmediato falló, requiere confirmación de email',
        loginError,
      );
