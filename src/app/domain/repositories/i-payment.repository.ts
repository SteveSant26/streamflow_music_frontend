...
export abstract class IPaymentRepository {
  ...
  abstract createSubscription(dto: CreateSubscriptionDto): Observable<Subscription>;
  abstract updateSubscription(subscriptionId: string, dto: UpdateSubscriptionDto): Observable<Subscription>;
  abstract cancelSubscription(subscriptionId: string): Observable<void>;
  abstract reactivateSubscription(subscriptionId: string): Observable<Subscription>;
  ...
  abstract addPaymentMethod(userId: string, dto: AddPaymentMethodDto): Observable<PaymentMethod>;
  abstract removePaymentMethod(paymentMethodId: string): Observable<void>;
  abstract setDefaultPaymentMethod(userId: string, paymentMethodId: string): Observable<void>;
  ...
  // Pagos
  abstract createPaymentIntent(dto: CreatePaymentIntentDto): Observable<{ clientSecret: string; paymentIntentId: string }>;
  ...
  // Customer de Stripe
  abstract createCustomer(dto: CreateCustomerDto): Observable<{ customerId: string }>;
  abstract updateCustomer(customerId: string, data: Partial<CreateCustomerDto>): Observable<void>;

  // Cupones y descuentos
  abstract validateCoupon(couponCode: string): Observable<{ valid: boolean; discount?: number; description?: string }>;
  abstract applyCoupon(dto: ApplyCouponDto): Observable<Subscription>;

  // Checkout y billing portal
  abstract createCheckoutSession(dto: StripeCheckoutSessionDto): Observable<{ url: string; sessionId: string }>;
  abstract createBillingPortalSession(customerId: string, dto: BillingPortalDto): Observable<{ url: string }>;
