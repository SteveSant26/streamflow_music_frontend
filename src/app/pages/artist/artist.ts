  ...
  template: `
    <div class="artist-container p-6">
      <div class="artist-header mb-8" *ngIf="artist">
        <!-- Artist Hero Section -->
        <div class="relative rounded-2xl overflow-hidden text-white p-8 mb-6 transition-all duration-1000" 
      ...
      <div class="artist-content" *ngIf="artist">
        ...
        <section class="popular-songs mb-8">
          ...
          <div class="bg-white rounded-xl shadow-sm border border-gray-200">
            <div *ngFor="let song of artist.popularSongs; let i = index" 
        ...
        <section class="awards mb-8" *ngIf="artist.awards.length > 0">
          ...
          <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div *ngFor="let award of artist.awards" 
                   class="flex items-center p-3 bg-gray-50 rounded-lg">
                <div class="w-2 h-2 bg-yellow-500 rounded-full mr-3"></div>
                <div>
                  <h4 class="font-medium">{{ award.name }}</h4>
                  <p class="text-sm text-gray-600">{{ award.year }} - {{ award.category }}</p>
                </div>
              </div>
            </div>
          </div>
...
export class ArtistComponent implements OnInit {
  ...
  private applyFallbackGradient() {
    if (this.artist) {
      this.artist.gradient = 'linear-gradient(135deg, #FF6B6B 0%, #4ECDC4 100%)';
      console.log('Aplicado gradiente de fallback');
    }
  }

  private findDominantColor(colors: Array<{r: number, g: number, b: number}>) {
...
La banda ha lanzado múltiples álbumes exitosos, incluyendo su álbum debut homónimo (2001), "Demon Days" (2005), "Plastic Beach" (2010), "Humanz" (2017), "The Now Now" (2018), "Song Machine" (2020) y "Cracker Island" (2023). Sus videos musicales y presentaciones en vivo utilizan tecnología de vanguardia para dar vida a los personajes animados, creando experiencias visuales únicas.`,
      popularSongs: [
        ...
        { title: 'On Melancholy Hill', album: 'Plastic Beach', duration: '3:53' },
      ...
      awards: [
        { name: 'Grammy Award', year: '2006', category: 'Best Pop Collaboration with Vocals - Feel Good Inc.' },
        { name: 'BRIT Award', year: '2006', category: 'Best British Group' },
        { name: 'MTV Video Music Award', year: '2005', category: 'Best Group Video - Feel Good Inc.' },
        { name: 'Q Award', year: '2005', category: 'Best Video - Feel Good Inc.' },
        { name: 'Ivor Novello Award', year: '2002', category: 'Best Contemporary Song - Clint Eastwood' },
