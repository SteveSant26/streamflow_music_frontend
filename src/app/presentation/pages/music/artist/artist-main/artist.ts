...
export class ArtistComponent implements OnInit {
  private getArtistByIdUseCase = inject(GetArtistByIdUseCase);
  private getAlbumsByArtistUseCase = inject(GetAlbumsByArtistUseCase);
  ...
  private async loadArtistAlbums(artistId: string): Promise<void> {
    ...
    try {
      this.getAlbumsByArtistUseCase.execute({ artistId, limit: 20 }).subscribe({
        next: (albums) => {
