  ...
  template: `
    <div class="subscription-management-container">
      <div class="page-header">
        ...
        <button class="billing-portal-btn" (click)="openBillingPortal()" [disabled]="loading()">
      ...
      <div class="content-grid" *ngIf="!loading(); else loadingTemplate">
        ...
        <div class="subscription-info-card" *ngIf="subscription()">
          ...
          <div class="plan-details">
            <div class="plan-name">{{ currentPlan()?.name || 'Plan Premium' }}</div>
            <div class="plan-price">{{ formatPrice(currentPlan()?.price || 0) }}/{{ currentPlan()?.interval === 'month' ? 'mes' : 'año' }}</div>
          ...
          <div class="subscription-dates">
            ...
            <div class="date-info" *ngIf="subscription()?.trialEnd && isOnTrial()">
          ...
          <div class="subscription-actions">
            ...
            <button 
              ...
              [disabled]="subscription()?.status !== 'active' || subscription()?.canceledAt"
        ...
        <div class="payment-methods-card">
          ...
          <div class="payment-methods-list" *ngIf="paymentMethods().length > 0; else noPaymentMethods">
            <div 
              *ngFor="let method of paymentMethods()" 
              ...
              <div class="card-info">
                ...
                <div class="card-expiry">{{ method.card?.expMonth }}/{{ method.card?.expYear }}</div>
              </div>
              <div class="method-actions">
                <span class="default-badge" *ngIf="method.isDefault">Por defecto</span>
                <button class="manage-btn" (click)="openBillingPortal()">Gestionar</button>
              </div>
            </div>
          </div>
        ...
        <div class="billing-info-card" *ngIf="upcomingInvoice()">
          ...
          <div class="invoice-info">
            <div class="amount">{{ formatPrice(upcomingInvoice()?.amount || 0) }}</div>
            <div class="due-date">Vence: {{ formatDate(upcomingInvoice()!.createdAt) }}</div>
          </div>
        </div>
      </div>
...
export class SubscriptionManagementComponent implements OnInit {
  // Computed properties
  readonly subscription = computed(() => this.paymentStateService.subscription());
  readonly currentPlan = computed(() => this.paymentStateService.currentPlan());
  readonly paymentMethods = computed(() => this.paymentStateService.paymentMethods());
  ...
  readonly upcomingInvoice = computed(() => this.paymentStateService.upcomingInvoice());
  ...
  async cancelSubscription(): Promise<void> {
    ...
    const confirmed = confirm('¿Estás seguro de que deseas cancelar tu suscripción? Tu acceso premium continuará hasta el final del período de facturación actual.');
    
    if (confirmed) {
      this.cancelSubscriptionUseCase.execute(subscription.id).subscribe({
        next: () => {
          alert('Tu suscripción ha sido cancelada. Seguirás teniendo acceso premium hasta el final del período actual.');
        },
        error: (error) => {
          console.error('Error al cancelar suscripción:', error);
          alert('Hubo un error al cancelar tu suscripción. Por favor, inténtalo de nuevo.');
        }
      });
    }
  }
  ...
  async openBillingPortal(): Promise<void> {
    ...
    try {
      const result = await this.createBillingPortalUseCase.execute(
      ...
      alert('Hubo un error al abrir el portal de facturación. Por favor, inténtalo de nuevo.');
    }
  }
